// MCP plugin frontend - Commercial-grade UI (Raycast/Composio style)
// Dark surface hierarchy, hairline borders, proper typography, tool browser,
// server cards with health indicators, animations, empty states, loading skeletons.
//
...
interface MCPServer {
  ...
  connected: boolean; toolCount: number; resourceCount: number;
  serverInfo?: { name: string; version: string };
}
interface MCPDetail extends MCPServer {
...
interface MCPPrompt { name: string; description: string; arguments: Array<{ name: string; description?: string; required?: boolean }>; }
interface MCPLogEntry { level: string; server: string; message: string; timestamp: number; }

// --- Design Tokens (Raycast-inspired dark surface hierarchy) ---
...
function _el(tag: string, css?: string, text?: string): HTMLElement {
...
function _elHtml(tag: string, css: string, html: string): HTMLElement {
  const e = document.createElement(tag);
  if (css) e.style.cssText = css;
  e.innerHTML = html;
  return e;
...
function _transition(props?: string): string {
...
function _dotClass(connected: boolean): string {
  return "mcp-dot " + (connected ? "mcp-dot-connected" : "mcp-dot-disconnected");
}

// --- Loading Skeleton ---

function _skeletonCards(count: number): HTMLElement {
...
function _emptyState(title: string, desc: string, btnHtml?: string): HTMLElement {
  const wrap = _el("div", [
...
function _transportStyle(transport: string): string {
...
function _serverCard(s: MCPServer, isSelected: boolean, h: CardHandlers): HTMLElement {
  ...
  if (!isSelected) {
    ...
    card.addEventListener("click", function() { h.onSelect(s.id); });
  }
  ...
  function mkBtn(icon: string, title: string, danger?: boolean) {
...
function _toolCard(t: MCPTool, expanded: boolean, onToggle: () => void): HTMLElement {
...
function _resourceRow(r: MCPResource): HTMLElement {
  const row = _el("div", [
...
function _promptRow(p: MCPPrompt): HTMLElement {
  const row = _el("div", [
    "padding:10px 14px;background:" + T.card + ";",
    "border:1px solid " + T.hairlineSoft + ";border-radius:" + T.rMd + ";margin-bottom:6px;",
    _transition(),
...
class MCPPage {
  ...
  private _wsReconnectTimer: number | null = null;
  ...
  private _connectWebSocket(): void {
    ...
    try {
      ...
      this._ws.onclose = () => {
        this._ws = null;
        this._wsReconnectTimer = window.setTimeout(() => this._connectWebSocket(), 3000);
      };
      this._ws.onerror = () => {};
    } catch {}
  }
  ...
  private _addLogEntry(entry: MCPLogEntry): void {
  ...
  private async _load(silent?: boolean): Promise<void> {
    if (!silent) { this._loading = true; this._buildList(); }
    try {
  ...
  private _buildDetail(d: MCPDetail): void {
    ...
    function renderTab(idx: number): void {
      self._activeTab = idx;
      const tabs = tabContainer.querySelectorAll(".mcp-tab");
      tabs.forEach((t, i) => { t.className = "mcp-tab" + (i === idx ? " mcp-tab-active" : ""); });
      ...
      if (idx === 0) {
        ...
        const searchIcon = _elHtml("div", "position:absolute;left:10px;top:50%;transform:translateY(-50%);pointer-events:none;",
        ...
        const renderTools = (q: string): void => {
  ...
  private _showForm(edit?: MCPServer): void {
    ...
    const saveBtn = new ui.Button({
      ...
      onClick: async function() {
        ...
        else body.url = (urlInput.element as HTMLInputElement).value;
        ...
        try {
          let resp: Response;
          if (edit) {
            resp = await fetch("/api/mcp/servers/" + edit.id, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
          } else {
  ...
  async _deleteServer(id: string): Promise<void> {
  ...
  async _reconnect(id: string): Promise<void> {
    try {
...
const page = new MCPPage();
(window as any)._mcpPage = page;
