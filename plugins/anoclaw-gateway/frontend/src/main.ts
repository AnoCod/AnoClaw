...
interface PlatformDef {
  id: string; name: string; icon: string; color: string; colorSoft: string;
  fields: Array<{ key: string; label: string; type: string; placeholder: string; required: boolean; help?: string }>;
  setupSteps?: string[];
}
...
function platformIcon(id: string): string { return ICONS[id] || ''; }

class GatewayPage {
  name = 'gateway';
  container: HTMLElement;
  private _connections: GatewayConnection[] = [];
  private _inbox: InboxMessage[] = [];
  private _health: any = null;
  private _templates: MessageTemplate[] = [];
  private _retryQueue: RetryItem[] = [];
  private _ws: WebSocket | null = null;
  private _wsReconnectTimer: any = null;
  private _wsConnected = false;
  private _activeTab: 'inbox' | 'health' | 'templates' | 'retry' = 'inbox';
  private _searchQuery = '';
  private _searchPlatform = '';
  private _searchResults: InboxMessage[] | null = null;
  private _selectedMessage: InboxMessage | null = null;
  private _wizardConnectionId: string | null = null;
  ...
  onExit(): void { this._disconnectWebSocket(); }
      ...
      this._ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'gateway:message') {
            this._inbox.push(msg as InboxMessage);
  ...
  private async _load(): Promise<void> {
    try { const r = await fetch('/api/gateway/connections'); if (r.ok) { const d = await r.json(); this._connections = d.connections || []; } } catch { this._connections = []; }
    try { const r = await fetch('/api/gateway/inbox'); if (r.ok) { const d = await r.json(); this._inbox = d.messages || []; } } catch { this._inbox = []; }
    try { const r = await fetch('/api/gateway/health'); if (r.ok) { this._health = await r.json(); } } catch { this._health = null; }
    try { const r = await fetch('/api/gateway/templates'); if (r.ok) { const d = await r.json(); this._templates = d.templates || []; } } catch { this._templates = []; }
    try { const r = await fetch('/api/gateway/retry-queue'); if (r.ok) { const d = await r.json(); this._retryQueue = d.queue || []; } } catch { this._retryQueue = []; }
    this._render();
  }
  ...
  private _render(): void {
    ...
    inner.querySelectorAll('.gw-platform-card').forEach(card => {
      card.addEventListener('click', () => this._showAdd((card as HTMLElement).dataset.platform!));
    });
    // Bind connection actions
    Array.from(inner.querySelectorAll('[data-act]')).forEach(el => {
      const h = el as HTMLElement; const act = h.dataset.act; const id = h.dataset.id;
    ...
    inner.querySelectorAll('.gw-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        this._activeTab = (tab as HTMLElement).dataset.tab as any;
  ...
  private _renderInboxTabHtml(): string {
    const platformColors: Record<string, { bg: string, fg: string }> = {
    ...
    // Use search results if available, otherwise show all messages
    ...
    const bubbleList = msgs.length === 0
      ...
      : `<div class="gw-inbox-list">
          ${msgs.map(m => {
            ...
            const mediaTag = m.media_type && m.media_type !== 'text'
              ? `<span class="gw-msg-media" style="background:${pc.bg};color:${pc.fg};">${m.media_type}</span>` : '';
  ...
  private _renderMessageDetail(msg: InboxMessage): string {
  ...
  private _bindInboxCompose(): void {
    ...
    const searchInput = this.container.querySelector('#gw-search-input') as HTMLInputElement;
    const searchPlatform = this.container.querySelector('#gw-search-platform') as HTMLSelectElement;
    ...
    if (searchBtn && searchInput && searchPlatform) {
      const doSearch = async () => {
        ...
        try {
          ...
          if (r.ok) { const d = await r.json(); this._searchResults = d.messages || []; }
    ...
    // Message click -> detail view
    this.container.querySelectorAll('.gw-msg-bubble').forEach(bubble => {
      bubble.addEventListener('click', () => {
        const idx = parseInt((bubble as HTMLElement).dataset.idx || '-1');
    ...
    const input = this.container.querySelector('#gw-compose-input') as HTMLTextAreaElement;
    const target = this.container.querySelector('#gw-compose-target') as HTMLSelectElement;
  ...
  private _bindTemplatesTab(): void {
    ...
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        ...
        form.querySelector('.gw-form-save')?.addEventListener('click', async () => {
          const name = (document.getElementById('gw-tpl-name') as HTMLInputElement)?.value || '';
          const platform = (document.getElementById('gw-tpl-platform') as HTMLSelectElement)?.value || 'any';
          const content = (document.getElementById('gw-tpl-content') as HTMLTextAreaElement)?.value || '';
          const category = (document.getElementById('gw-tpl-category') as HTMLInputElement)?.value || 'general';
  ...
  private _showWizard(connectionId: string): void {
    ...
    panel.id = 'gw-wizard-panel'; panel.className = 'gw-wizard';
    panel.innerHTML = `
      <div class="gw-detail-header">
        <div>
  ...
  private _showAdd(platform: string): void {
    ...
    form.querySelector('.gw-form-save')?.addEventListener('click', async () => {
      ...
      p.fields.forEach(f => config[f.key] = (document.getElementById(`gw-f-${f.key}`) as HTMLInputElement)?.value || '');
...
const page = new GatewayPage();
(window as any)._gwPage = page;
