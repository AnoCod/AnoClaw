...
// Purple-accent cinema style. Auto-builds option buttons with single/multi-select.
...
export class AskUserCard {
  ...
  private _injectStyles(): void {
    ...
    s.textContent = `.ui-askusercard{margin-bottom:12px;border-radius:4px;background:rgba(124,58,237,.04);overflow:hidden}
.ui-askusercard-header{display:flex;align-items:center;gap:8px;padding:10px 14px;font-size:10px;color:rgba(167,139,250,.35);letter-spacing:1px;text-transform:uppercase}
.ui-askusercard-dot{width:4px;height:4px;border-radius:50%;flex-shrink:0;background:rgba(167,139,250,.4)}
...
.ui-askusercard-opt{display:flex;flex-direction:column;gap:2px;padding:8px 12px;border:1px solid rgba(124,58,237,.15);border-radius:6px;background:rgba(124,58,237,.04);color:var(--color-text-primary);cursor:pointer;font-family:inherit;font-size:12px;text-align:left;transition:border-color .15s,background .15s}
.ui-askusercard-opt:hover{border-color:rgba(124,58,237,.35);background:rgba(124,58,237,.08)}
