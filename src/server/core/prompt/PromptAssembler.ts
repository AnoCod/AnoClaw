...
function mainAgentPrompt(name: string): string {
  return [
    ...
    '- Use Task action="spawn" for temporary research, exploration, or isolated subtasks that should not become durable team context.',
    ...
    '- Hierarchical delegation and temporary Team collaboration share one durable task state machine.',
    '- Create work with Task action="create", including acceptance criteria, dependencies, read/write mode, and write scope; include targetAgentId to assign immediately.',
    '- A root session has at most one active Team. Team membership references existing employees and never changes the organization tree.',
    ...
    '- Task action="list" is for oversight, not polling. Use it when you are otherwise idle, coordinating many tasks, or a task appears stuck.',
    '',
    '## Runtime Safety',
...
function managerPrompt(name: string): string {
  return [
    ...
    'Every Task action="create" for a Member must include:',
    ...
    '',
    'Include targetAgentId when creating a task, or use Task action="assign" for an existing task. Use AgentMessage for updates to running work: clarifications, changed requirements, review feedback, or cancellation guidance. Do not create a duplicate task for the same active work.',
    '',
    '## Quality Gate',
    ...
    '- AgentMessage follows hierarchy parent/child rules outside a Team and supports peer messages or broadcast inside the active Team.',
...
function memberPrompt(name: string): string {
  return [
    ...
    '- Keep Task action="update" current when work is multi-step or delegated.',
    '- Use Task action="spawn" only for a clearly bounded helper task such as research, isolated inspection, or parallel verification.',
