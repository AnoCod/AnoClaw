 ...
 * presses Enter or clicks SEND → onSend callback fires → caller routes to
 * ConversationViewModel → SessionAgent.sendMessage via WebSocket.
 *
...
export class InputPanel {
  ...
  constructor() {
    ...
    this._modeSelector = new ModeSelector('auto', 'normal', true);
    ...
    this._modeSelector.onRunningModeChange = (runningMode) => {
      convVM.setRunningMode(runningMode);
    };
  ...
  // Build DOM: attachments bar (above) → textarea (mid) → button bar (below)
  ...
  // ── Textarea: auto-grows, Enter to send, arrow keys/Esc in slash popup ──
  ...
  private _makeTextarea(): HTMLTextAreaElement {
    ...
    // caused by DOM updates during streaming — the SlashCommandPanel's own
    // click-outside handler covers the case where the user clicks a non-focusable
    // element.
    ta.addEventListener('focusout', (e) => {
      const related = (e as FocusEvent).relatedTarget as HTMLElement | null;
      if (!related) return; // involuntary blur or click on non-focusable — let click handler decide
      // Focus moved within the input area — keep slash panel open
      if (this.element.contains(related)) return;
      // Focus moved to slash panel popup — keep it open (defensive; items aren't focusable today)
      if (this._slashPanel.containsElement(related)) return;
      // Genuine focus change to a focusable element elsewhere — close after a tick
      setTimeout(() => {
  ...
  // ── Attach button — uploads local files and sends them as attachments to the agent.
  // This is NOT for changing the workspace directory (see right-edge Files panel "Change…" button).
  ...
  // Rebuild attachment chips DOM from _attachments array — each chip has icon, name, remove button
  ...
  setStreaming(v: boolean): void {
    ...
    this._sendBtn.style.display = '';          // always visible — user can interject
