...
export class WebSearchTool extends Tool {
  ...
  description(): string {
    return 'Search the web for up-to-date information. Supports domain filtering. Uses multiple search backends.';
  }
  ...
  private async _tryDdgLite(encoded: string, extSignal?: AbortSignal): Promise<Array<{ title: string; url: string; snippet: string }> | null> {
    try {
      ...
      if (extSignal) {
        if (extSignal.aborted) { ctrl.abort(); clearTimeout(t); }
        else extSignal.addEventListener('abort', () => ctrl.abort(), { once: true });
      }
      const resp = await webFetch(`https://lite.duckduckgo.com/lite/?q=${encoded}`, { signal: ctrl.signal });
      clearTimeout(t);
  ...
  private async _tryDdgHtml(encoded: string, extSignal?: AbortSignal): Promise<Array<{ title: string; url: string; snippet: string }> | null> {
    try {
      ...
      const resp = await webFetch(`https://html.duckduckgo.com/html/?q=${encoded}`, { signal: ctrl.signal });
      clearTimeout(t);
      if (!resp.ok) return null;
      const html = await resp.text();
      const results: Array<{ title: string; url: string; snippet: string }> = [];
  ...
  private async _tryBing(encoded: string, extSignal?: AbortSignal): Promise<Array<{ title: string; url: string; snippet: string }> | null> {
    try {
      ...
      const resp = await webFetch(`https://www.bing.com/search?q=${encoded}&count=10`, { signal: ctrl.signal });
      clearTimeout(t);
      if (!resp.ok) return null;
      const html = await resp.text();
