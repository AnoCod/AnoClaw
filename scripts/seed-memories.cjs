...
代码注释和标识符仍然使用英文（项目规范）。面向 AI 的仓库级文档以 AGENTS.md / anoclaw.md 为准。`,
  },
  {
...
### Phase 4: VERIFY
- Build (必须通过) → Tests → 功能验证 → 行数检查 → AGENTS.md / anoclaw.md 同步
...
新增重要入口或架构变化时，必须更新 AGENTS.md、anoclaw.md 或 docs/ 下的对应说明。`,
  },
  {
...
- 插件通过 ExtensionPoints 扩展内核`,
  ...
  {
    dir: 'team', name: 'codex-doc-sync', type: 'project', scope: 'team',
    description: 'Codex 项目文档同步规则',
    content: `## Codex Project Documentation Sync Rules

每次代码变更后必须检查 AGENTS.md、anoclaw.md 或 docs/ 下的项目说明是否需要同步更新。

### 检查清单
1. 新增重要模块 → 更新 AGENTS.md、anoclaw.md 或 docs/ 中的架构/路由说明
2. 删除文件 → 移除过时说明
3. 拆分文件 → 更新旧文件和新文件的职责描述
4. 职责变更 → 更新 Quick Task Routing 或对应开发约定
5. 新增系统 → 添加路由条目

### 验证
\`rg <new-file-basename> AGENTS.md anoclaw.md docs src\` 确认关键说明已同步。
