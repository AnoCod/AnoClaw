...
> 安全边界：Worker Thread 提供的是故障与内存对象隔离，不是权限沙箱。插件入口仍是拥有当前用户权限的 Node.js 本机代码，可以直接导入 `fs`、`child_process`、网络模块并访问宿主可访问的资源。安装或启用第三方插件等同于运行该来源的本机程序；只使用经过审查且可信的来源。`api.fs` 的 workspace 限制只约束该 API，不会拦截插件自行导入的 Node.js 文件接口。
